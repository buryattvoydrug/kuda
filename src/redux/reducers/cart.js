// const initialState={
//   // itemsCart:{}
//   // totalCount:0
// }

// const getTotalPrice=arr=>arr.reduce((sum,obj)=>obj.price+sum,0)

const cart = (
  state = { itemsCart: JSON.parse(localStorage.getItem("itemsCart") || "[]") },
  action
) => {
  switch (action.type) {
    case 'ADD_PIZZA_CART':
      return { itemsCart: action.payload.itemsCart };
    case 'REMOVE_CART_ITEM':
      return { itemsCart: action.payload.itemsCart };
    default:
      return state;
  }
};

// const cart=(state=initialState,action)=>{
//   switch (action.type) {
//     case 'ADD_PIZZA_CART':{
//       console.log(action.payload.post)
//       const currentPizzaItems=!state.itemsCart[action.payload.post.id]
//                               ? [action.payload.post]
//                               : [...state.itemsCart[action.payload.post.id].itemsCart,action.payload.post]
    
//        console.log(currentPizzaItems)
      
//       const newItems={
//         ...state.itemsCart,
//         [action.payload.post.id]:{
//           itemsCart:currentPizzaItems,
//           totalPrice:getTotalPrice(currentPizzaItems),
//         },
//       }
     
//       const totalCount=newItems.length
//       // const totalCount=Object.keys(newItems).reduce(
//       //   (sum,key)=>newItems[key].itemsCart.length+sum,
//       //   0
//       // );

//       console.log(newItems,totalCount)
      
//       return{
//         ...state,
//         itemsCart:newItems,
//         totalCount,
//       };
//     }
//     case 'CLEAR_CART':{
//       return {
//         items:{},
//         totalCount:0,
//       }
//     }
//     case 'REMOVE_CART_ITEM':{
//       const newItems={
//         ...state.items
//       }
//       const currentTotalPrice=newItems[action.payload].totalPrice;
//       const currentTotalCount=newItems[action.payload].items.length;
//       delete newItems[action.payload];
//       return{
//         ...state,
//         items:newItems,
//         // totalCount:newItems.length,
//         totalPrice:state.totalPrice-currentTotalPrice,
//         totalCount:state.totalCount-currentTotalCount,
        
//       }
//     }
//     case 'PLUS_CART_ITEM':{

//         const newObjItems=[...state.items[action.payload].items,state.items[action.payload].items[0]]
//         const newItems={
//           ...state.items,
//           [action.payload]:{
//             items:newObjItems,
//             totalPrice:getTotalPrice(newObjItems),

//             },
//           }

//           const totalCount=Object.keys(newItems).reduce(
//             (sum,key)=>newItems[key].items.length+sum,
//             0
//           );
//           const totalPrice=Object.keys(newItems).reduce(
//             (sum,key)=>newItems[key].totalPrice+sum,
//             0
//           );

//           return{
//               ...state,
//               items:newItems,
//               totalCount,
//               totalPrice
//           }}
//     case 'MINUS_CART_ITEM':{
//       const oldItems= state.items[action.payload].items;
//       const newObjItems=oldItems.length > 1 ? state.items[action.payload].items.slice(1) : oldItems;

//       const newItems={
//         ...state.items,
//         [action.payload]:{
//            items:newObjItems,
//           totalPrice:getTotalPrice(newObjItems),

//           },
//         }

//       const totalCount=Object.keys(newItems).reduce(
//       (sum,key)=>newItems[key].items.length+sum,
//       0
//     );
//     const totalPrice=Object.keys(newItems).reduce(
//       (sum,key)=>newItems[key].totalPrice+sum,
//       0
//     );
//       return{
//         ...state,
//         items:newItems,
//         totalCount,
//         totalPrice
//       }}
//     default:
//       return state;
//   }
  
// }
export default cart;